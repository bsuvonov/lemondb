...
public:
  ...
  public:
    ...
    DatumIterator base() const {
      return it; // Return the underlying DataIterator
    }
...
public:
  ...
  /**
   * Erase a record by iterator, returns iterator to the next element after
   * erased one
   * @param it: iterator pointing to the record to be erased
   * @return iterator to the next element after the erased one
   */
  Iterator erase(Iterator it) {
    keyMap.erase(it->key()); // Use key() to get the key through the proxy
    std::swap(*(it.base()), *(this->data.end() - 1));
    this->data.pop_back();
    return Iterator((it.base()), this);
    // return Iterator(this->data.erase(it.base()), this);
  }

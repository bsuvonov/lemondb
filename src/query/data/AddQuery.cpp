...
QueryResult::Ptr AddQuery::execute() {
  ...
  std::vector<Table::FieldIndex> field_id_list(this->getOperands().size() - 1);
  size_t count = 0;
  try {
    auto &table = db[this->tableName()];
    auto result = initCondition(table);

    for (std::size_t j = 0; j < this->getOperands().size() - 1; ++j) {
      field_id_list[j] = (table.getFieldIndex(this->getOperands()[j]));
    }
    Table::FieldIndex const destid = table.getFieldIndex(
        this->getOperands()[this->getOperands().size() - 1]);
    auto op = [&field_id_list, &destid](Table::Object object, void *data) {
      Table::ValueType res = 0;
...
std::string AddQuery::toString() {
  return "QUERY = ADD \"" + this->tableName() + "\"";
}
